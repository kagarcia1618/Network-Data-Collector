...
def napalm_ssh(driver, node, cli_cmd, username, password, mode, custom):
    ...
    password - device login passwordpytho
    ...
    device = net_driver(str(node.primary_ip4).split('/')[0], username, password, optional_args={'global_delay_factor': 2, 'secret': password})
    try:
        device.open()
        output = device.cli(cli_cmd)
        device.close()
        wr_file = open( directory + node.name.split(":")[0] + '_' + str(node.primary_ip4).split('/')[0] + '_' + timestamp + '.' + mode, 'w' )
        for i in output:
            wr_file.write( i + '\n' + output[i] + '\n')
        wr_file.close()
