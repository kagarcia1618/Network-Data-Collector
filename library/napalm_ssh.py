...
def napalm_ssh(driver,node,cli_cmd,username,password,mode):
    ...
    device = net_driver(node[2], username, password, optional_args={'global_delay_factor': 2})
    try:
        device.open()
        ...
        for i in output:
            wr_file.write( i + '\n' + output[i] + '\n')
        wr_file.close()
        return print(f'  {node[1]} {mode} via napalm ssh data collection success!')
    except (ConnectionException) as error:
        wr_file = open( 'logs/' + node[1] + '_' + node[2] + '_' + timestamp + '.' + mode, 'w' )
        wr_file.write(str(error))
        wr_file.close()
        return print(f'  {node[1]} {mode} via napalm ssh data collection failed!')
